...
public class l002_directedgraph {
    ...
    public static ArrayList<Integer> kahnsAlgo(ArrayList<Edge>[] graph) {
        ...
        //O(E)
        for (int i = 0; i < N; i++) { // FIL THE INDEGREE ARRAY WITH VERTICES
            for (Edge e : graph[i]) {
                indegree[e.v]++;
            }
        } 
        ...
        //O(V)
        for (int i = 0; i < N; i++)     //ADD VERTICES IN QUEUE IF INDEGREE O ELSE NO SORTING POSSIBLE 
            if (indegree[i] == 0)
                que.addLast(i);

        //O(E + V) 
        while (que.size() != 0) {
            int rvtx = que.removeFirst(); //remove the first element in queue
            ans.add(rvtx);                  // add in the anser

            for (Edge e : graph[rvtx]) {  // if rvtx 's vertex value become 0 then it add it into the que
                if (--indegree[e.v] == 0) // indegree 0 means all previous value in que depend on that vertex will be processed if that verxte get processed in que;
                    que.addLast(e.v);
            }
        }
